...
class ProductSimilarity
{
    ...
    public function __construct(array $products)
    {
        ...
        $this->priceHighRange = max(array_column($products, 'gross_price'));
    }
    ...
    {
        $productAFeatures = $productA['color'];
        $productBFeatures = $productB['color'];
        return array_sum([
            (Similarity::hamming($productAFeatures, $productBFeatures) * $this->featureWeight),
            (Similarity::euclidean(
                Similarity::minMaxNorm([$productA['gross_price']], 0, $this->priceHighRange),
                Similarity::minMaxNorm([$productB['gross_price']], 0, $this->priceHighRange)
            ) * $this->priceWeight),
            (Similarity::jaccard($productA['type'], $productB['type']) * $this->categoryWeight)
